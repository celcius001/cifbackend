...
app.post(
  ...
  (req, res) => {
    ...
    request.query(query, (err, result) => {
      ...
      res
        .status(201)
        .json({ message: 'CIF record added successfully', insertId: result.rowsAffected[0] });
    });
  }
);
...
app.get('/cif', (req, res) => {
  const query = 'SELECT [CIF Key], [Member Name], Spouse, [Sitio Purok], Barangay, Municipality, District, [Signature Of Member], Picture, DateCreated FROM CIF;';
...
app.get('/cif/:cifKey', (req, res) => {
  ...
  const query = 'SELECT [CIF Key], [Member Name], Spouse, [Sitio Purok], Barangay, Municipality, District, [Signature Of Member], Picture, DateCreated FROM CIF WHERE [CIF Key] = @cifKey;';
  executeQuery(
    query,
...
app.get('/cif/search/:memberName', (req, res) => {
  ...
  const query = 'SELECT [CIF Key], [Member Name], Spouse, [Sitio Purok], Barangay, Municipality, District, [Signature Of Member], Picture, DateCreated FROM CIF WHERE [Member Name] = @memberName;'; // Exact match
  executeQuery(
    query,
